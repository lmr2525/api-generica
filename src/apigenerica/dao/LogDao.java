 ...
 * Lectura/escritura directa en JSON (sin driver JDBC, sin subprocesos). - 100%
 * fiable entre reinicios: los datos sobreviven siempre. - Formato: una línea
 * JSON por registro para append atómico.
 *
 * STORAGE SECUNDARIO (best-effort): Paradox via ParadoxWorker - Se intenta
 * también escribir en logs.db para compatibilidad. 
...
public class LogDao {
    ...
    public static List<String[]> obtenerTodos() {
        ...
        for (int id : ids) {
            lista.add(combinado.get(id));
        }
        return lista;
    }

    /**
     * Busca logs por usuario, operación o tabla (búsqueda simple
     * case-insensitive). Devuelve siempre en orden descendente por ID.
     */
    ...
    private static void persistirFilaEnJSON(String[] fila) {
        try (FileOutputStream fos = new FileOutputStream(JSON_FILE, true); PrintWriter pw = new PrintWriter(
    ...
    private static void guardarTodoElBufferEnJSON() {
        try (PrintWriter pw = new PrintWriter(
            ...
            synchronized (bufferMemoria) {
                ...
                for (int id : ids) {
                    pw.println(filaALineaJSON(bufferMemoria.get(id)));
                }
            }
            pw.flush();
        } catch (IOException e) {
